...
Original file is located at
    https://colab.research.google.com/drive/1IMZf5r-I2EnTK41ASnoFn8pP4-67EhRp
...
def three_layer_convnet(x, params):
    ...
    ############################################################################
    padding_with_2 = tf.constant([[0,0],[2,2],[2,2],[0,0]])
    x_with_pad = tf.pad(x, padding_with_2, mode = 'constant')
    conv1 = tf.nn.conv2d(x_with_pad, conv_w1, strides = [1,1,1,1], padding = 'VALID') + conv_b1
    relu1 = tf.nn.relu(conv1)
    
    padding_with_1 = tf.constant([[0,0],[1,1],[1,1],[0,0]])
    relu1_with_pad = tf.pad(relu1, padding_with_1, mode = 'constant')
    conv2 = tf.nn.conv2d(relu1_with_pad, conv_w2, strides = [1,1,1,1], padding = 'VALID') + conv_b2
...
def kaiming_normal(shape):
    ...
    print "RETURN: ",tf.random_normal(shape) * np.sqrt(2.0 / fan_in)
...
class ThreeLayerConvNet(tf.keras.Model):
    def __init__(self, channel_1, channel_2, num_classes):
        ...
        filters = [5,5]
        kernel_size = 1
        self.conv1 = tf.layers.Conv2D(channel_1, filters, kernel_size, padding = 'valid', kernel_initializer = initializer, activation = tf.nn.relu)
        
        filters = [3,3]
        
        self.conv2 = tf.layers.Conv2D(channel_2, filters, kernel_size, padding = 'valid', kernel_initializer = initializer, activation = tf.nn.relu)
    ...
    def call(self, x, training=None):
        ...
        ########################################################################
        padding_with_2 = tf.constant([[0,0],[2,2],[2,2],[0,0]])
        x = tf.pad(x, padding_with_2, mode = 'constant')
        x = self.conv1(x)
        padding_with_1 = tf.constant([[0,0],[1,1],[1,1],[0,0]])
        x = tf.pad(x, padding_with_1, mode = 'constant')
        x = self.conv2(x)
        x = tf.layers.flatten(x)
        scores = self.fc(x)
        ########################################################################
...
def train_part34(model_init_fn, optimizer_init_fn, num_epochs=1):
    ...
    with tf.device(device):
        ...
        print "->calling model_init_fn"
        scores = model_init_fn(x, is_training)
        print "->return from model_init_fn"
...
def model_init_fn(inputs, is_training):
    ...
    kernel_size = [3,3]
    filters = 50
    conv1 = tf.layers.conv2d(inputs, filters, kernel_size, [1,1], padding='same', kernel_initializer=initializer)

    relu1 = tf.nn.relu(conv1)

    pool1 = tf.layers.max_pooling2d(relu1, [2,2], [2,2])
    
    filters = 60
    conv2 = tf.layers.conv2d(pool1, filters, kernel_size, [1,1], padding='valid', kernel_initializer=initializer)
    
    relu2 = tf.nn.relu(conv2)

    pool2 = tf.layers.max_pooling2d(relu2, [2,2], [2,2])
    
    filters = 72
    conv3 = tf.layers.conv2d(relu2, filters, kernel_size, [1,1], padding='valid',kernel_initializer=initializer)

    relu3 = tf.nn.relu(conv3)

    pool3 = tf.layers.max_pooling2d(relu3, [2,2], [2,2])
    avg_pool = tf.layers.average_pooling2d(pool3, [5,5], [1,1])
    avg_pool = tf.layers.flatten(avg_pool)

    fc1 = tf.layers.dense(avg_pool, 50)
    fc2 = tf.layers.dense(fc1, 50)
    net = tf.layers.dense(fc2, num_classes, activation='softplus')
...
*Number of Filters*: 
    - Convulation Layer 1:50, activation :ReLU
    - Convolution Layer 2:60, activation :ReLU
    - Convolution Layer 3:70, activation :ReLU


*Pooling*: max-pooling of size 2x2
*Average Pooling*
*Fully connected Layers*:
    - Fully connected Layer 1: 50
    - Fully connected Layer 2: 50, activation = softplus
